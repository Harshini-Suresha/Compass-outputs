...
# === Step 4.5: Remove unwanted samples ===
remove_samples <- c("Green_A6_L001", "Green_A6_L002",
                    "Red_A6_L001", "Red_A6_L002",
                    "extreg_lymph_extreg1_L2")

expr_mat_log <- expr_mat_log[, !(colnames(expr_mat_log) %in% remove_samples)]

# === Step 5: Save heatmap with fixed scale (-4 to 4) and smaller cells ===
png("7777777_filtered.png", width = 2000, height = 1200, res = 200)
...
pheatmap(expr_mat_log,
         ...
         show_colnames = TRUE,
         fontsize = 8,
         cellwidth = 8,   # smaller squares
         cellheight = 8,  # smaller squares
         legend = TRUE,
         color = colorRampPalette(c("navy", "white", "firebrick3"))(100),
         breaks = seq(-4, 4, length.out = 101),  # fix scale at -4 to 4
